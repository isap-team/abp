			...
			if (options.UseSearchByFullName)
			{
				if (FullName.TryParseForSearch(searchValue, out FullName fullName))
				{
					...
					while (true)
					{
						...
						{
							expression = expression.And(CreateExpression(options.CaseSensitive, options.FirstNameProperty, fullName.FirstName));
						}
			...
			Expression<Func<TEntity, bool>> expression)
		{
			if (searchValue.EndsWith(" "))
			{
				...
				return CreateExpression(options.CaseSensitive, options.PropertyName, $"{trimmedValue} %")
					.Or(CreateExpression(options.CaseSensitive, options.PropertyName, $"% {trimmedValue}"))
					.Or(CreateExpression(options.CaseSensitive, options.PropertyName, $"% {trimmedValue} %"));
			}
