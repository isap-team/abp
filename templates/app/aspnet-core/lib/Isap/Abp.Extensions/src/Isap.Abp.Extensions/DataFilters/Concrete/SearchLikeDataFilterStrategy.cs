			...
			switch (searchValues.Count)
			{
				...
				default:
					return searchValues.Aggregate(PredicateBuilder.False<TEntity>(), (expr, searchValue) =>
						expr.Or(CreateExpression(selectLikeOptions, searchValue))
					);
			}
		}
			...
			if (options.UseSearchByFullName)
			{
				if (FullName.TryParseForSearch(searchValue, out FullName fullName))
				{
					...
					while (true)
					{
						...
						else
						{
							if (fullName.FirstName.IsNullOrEmpty() && fullName.LastName.EndsWith("%"))
								expression = CreateExpression(options.CaseSensitive, options.LastNameProperty, fullName.LastName.EnsurePrefix("%"))
										.Or(CreateExpression(options.CaseSensitive, options.FirstNameProperty, fullName.LastName.EnsurePrefix("%")))
										.Or(CreateExpression(options.CaseSensitive, options.MiddleNameProperty, fullName.LastName.EnsurePrefix("%")))
									;
							else
						...
						{
							expression = expression.And(fullName.MiddleName.IsNullOrEmpty()
									? CreateExpression(options.CaseSensitive, options.FirstNameProperty, fullName.FirstName)
										.Or(CreateExpression(options.CaseSensitive, options.MiddleNameProperty, fullName.FirstName))
									: CreateExpression(options.CaseSensitive, options.FirstNameProperty, fullName.FirstName))
								;
						}
						...
						{
							expression = expression.And(CreateExpression(options.CaseSensitive, options.MiddleNameProperty, fullName.MiddleName));
						}
					...
					if (searchByFullNameAlso)
						expression = expression.And(CreateExpression(options.CaseSensitive, options.PropertyName,
