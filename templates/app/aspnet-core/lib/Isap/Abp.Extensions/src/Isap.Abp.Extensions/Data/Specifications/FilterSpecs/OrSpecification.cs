...
{
	public class OrSpecification<TEntity>: FilterSpecificationBase<TEntity>
	{
		...
		public override Expression<Func<TEntity, bool>> IsSatisfiedBy()
		{
			...
			result = _specifications.Skip(1).Aggregate(result, (expression, specification) => PredicateExtensions.Or(expression, specification.IsSatisfiedBy()));
			return result;
		}
	}
}
